 ...
 *
 * The DHT11 is an atmospheric sensor that returns relative humidity in percent and
 * temperature in degrees Celsius.  The DHT11 has a simple 1-wire input.  The MCU
 * communicates with the DHT11 on this wire as follows:
 *
 * 1. MCU pulls the data line low for more than 18 ms
 * 2. DHT11 responds by first pulling the line low for 80 us and 80 us high
 * 3. DHT11 then proceeds to send 40 bits with 0 and 1 distinguished by the high time.
 *    The low proceeding the high portion of the pulse will be 50 us while the pulse length
 *    for the high time will be between 26 and 70 us with the longer pulse length
 *    representing a 1.
 *
 * @version 0.1
 *
 ...
 * order are:
 *
 * * High byte for relative humidity.  This will be the value to the left of the decimal.
 * * Low byte for relative humidity.  This is the value to the right of the decimal.
 ...
 * @return DHT11_ERROR_PARITY_CHECK_FAILED if parity byte indicates data corruption.
 */
