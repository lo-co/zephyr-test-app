...
static const struct gpio_dt_spec button = GPIO_DT_SPEC_GET_OR(USER_BTN, gpios, {0});
...
int8_t button_module_init()
{
    ...
    k_tid_t button_thread_id = k_thread_create(&button_thread,
                                btn_thread_stack,
                                K_THREAD_STACK_SIZEOF(btn_thread_stack),
                                button_task,
                                NULL,
                                NULL,
                                NULL, 7, 0,
...
static void button_task()
{
    ...
    while (1)
    {
        ...
        if (current_button_state != previous_button_state)
        {
            ...
            else if (k_cyc_to_ms_ceil32(k_cycle_get_32()-cnt_start) >= DEBOUNCE_TIME_MS)
            {
                ...
                else // BUTTON RELEASED
                {
                    ...
                    if (k_cyc_to_ms_ceil32(stop_time-btn_timer_start) >= BTN_HOLD_TIME_MS)
                    {
                        LOG_INF("Button hold time was exceeded.  Hold time is %d", k_cyc_to_ms_ceil32(stop_time-btn_timer_start));
                    }
                    btn_timer_start = 0;
                }

                // Make sure to reset the debounce timer once the event has been processed
                cnt_start = 0;
            }
        }
        else
